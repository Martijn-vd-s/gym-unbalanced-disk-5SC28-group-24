...
# =====================================================================
#  VARIANT: new_vis_3
#  Reward = gaussian terms (matching the notebook) + new z-component.
#  z formula (x = th, y = omega):
#     u = y - sin(x)*d
#     z = sin(u + pi/2) * p * 1/(q*sqrt(2*pi)) * exp(-0.5*(y/q)^2)
#     kept where:  sin(x)*d - pi <= y <= sin(x)*d + pi
#                  AND ( pi/3 <= x <= 3pi/2  OR  -3pi/2 <= x <= -pi/3 )  (middle removed)
#                  AND z >= 0
#  Params: p=40.4, d=10.04, q=4.18
#  All gaussian reward terms are kept exactly as before.
# =====================================================================
...
class UnbalancedDisk(gym.Env):
    ...
    def __init__(self, umax=3., dt = 0.025, render_mode='human'):
        ...
        # ---- parameters voor calculate_z (new_vis_3) ----
        self.P_VALUE = 40.4
        self.D_VALUE = 10.04
        self.Q_VALUE = 4.18
        # x-banden: reward alleen in twee vleugels; middenstuk (|x| < pi/3) weg
        self.X_INNER = np.pi / 3.0        # 1/3 * pi
        self.X_OUTER = 3.0 * np.pi / 2.0  # 3/2 * pi

        def _calculate_z_internal():
            """
            Berekent z volgens de nieuwe formule (new_reward_vis_2),
            gebruikmakend van de huidige th (=x) en omega (=y).

                u = y - sin(x)*d
                z = sin(u + pi/2) * p * 1/(q*sqrt(2*pi)) * exp(-0.5*(y/q)^2)

            Geldig waar:
              - band:  sin(x)*d - pi <= y <= sin(x)*d + pi
              - x in [pi/3, 3pi/2]  OF  x in [-3pi/2, -pi/3]   (midden weg)
              - z >= 0
            Anders -> 0.
            """
            # Afstand tot het hart van de golf (centerline y = sin(x)*d)
            u = self.omega - np.sin(self.th) * self.D_VALUE

            # Doorsnede van de rib (cosinus-bult via sin(.. + pi/2)), k = 1
            cross_section = np.sin(u + 0.5 * np.pi)

            # Gaussische envelope in ABSOLUTE y
            envelope = (1.0 / (self.Q_VALUE * (2 * np.pi) ** 0.5)) \
                       * np.exp(-0.5 * (self.omega / self.Q_VALUE) ** 2)

            z = cross_section * self.P_VALUE * envelope

            # Voorwaarde 1: band om de golf
            condition_y_lower = np.sin(self.th) * self.D_VALUE - np.pi
            condition_y_upper = np.sin(self.th) * self.D_VALUE + np.pi
            in_band = (self.omega >= condition_y_lower) & \
                      (self.omega <= condition_y_upper)

            # Voorwaarde 2: alleen in de twee x-vleugels (midden |x| < pi/3 weg)
            in_x_wings = ((self.th >= self.X_INNER) & (self.th <= self.X_OUTER)) | \
                         ((self.th <= -self.X_INNER) & (self.th >= -self.X_OUTER))

            # Voorwaarde 3: alleen niet-negatieve z
            z_nonneg = (z >= 0)

            return np.where(in_band & in_x_wings & z_nonneg, z, 0)
        ...
        # DE REWARD FUNCTIE (gaussians gelijk aan reward_function_for_plot uit de notebook)
        self.reward_fun = lambda self_instance: (
            # main_reward_component: piek op PI (bovenkant) en 0 hoeksnelheid
            + gaussian_2d(self_instance.err(self_instance.th, np.pi), self_instance.omega, 0, 0, 1, 1, 0.0, 2)
            # bottom_punishment_component: straf rond 0 rad (onderkant)
            - gaussian_2d(self_instance.err(self_instance.th, 0), self_instance.omega, 0, 0, 3, 3, 0.0, 40)
            # bottom_punishment_component_2: brede straf rond 0 rad
            - gaussian_2d(self_instance.err(self_instance.th, 0), self_instance.omega, 0, 0, 1, 12, 0.0, 60)
            # piek_reward_component (UIT, zoals in de notebook)
            # + gaussian_2d(self_instance.err(self_instance.th, np.pi), self_instance.omega, 0, 0, 0.07, 0.07, 0.0, 0.15)
            # piek_test
            + gaussian_2d(self_instance.err(self_instance.th, np.pi), self_instance.omega, 0, 0, 0.15, 0.15, 0.0, 0.5)

            # Control input penalty
            - 0.001 * self_instance.u**2

            # Z-component (nieuwe formule met x-vleugels + z>=0)
