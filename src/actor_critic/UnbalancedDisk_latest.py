...
class UnbalancedDisk(gym.Env):
    ...
    def __init__(self, umax=3.0, dt=0.025, render_mode="human", randomise=False):
        ...
        self.err = lambda current_th, target_th: abs(
            ((current_th - target_th + np.pi) % (2 * np.pi)) - np.pi
        )
        ...
        self.reward_fun = lambda self_instance: (
            ...
            +gaussian_2d(
                self_instance.err(self_instance.th, np.pi),
                self_instance.omega,
                0,
                0,
                1,
            ...
            - gaussian_2d(
                self_instance.err(self_instance.th, 0),
                self_instance.omega,
                0,
                0,
                3,
    ...
    def step(self, action):
        ...
        if self.time_to_become_stable >= 100:
            ...
            self.th_ref = np.deg2rad(self.th_set) * np.sin(
                2 * np.pi * 0.2 * t
            )  # move set-point at 0.2 Hz +- 15 deg
            self.step_t += 1
        else:
            self.th_ref = 0
        # self.th_ref = 0
    ...
    def reset(self, seed=None):
        ...
        self.th = (
            self.set_th
            if self.set_th is not None
            else np.random.uniform(-np.pi / 4, np.pi / 4)
        )
        self.omega = (
            self.set_omega
            if self.set_omega is not None
            else np.random.uniform(-1.0, 1.0)
        )
        self.u = 0
