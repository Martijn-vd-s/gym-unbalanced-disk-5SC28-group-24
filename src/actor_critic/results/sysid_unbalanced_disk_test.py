...
def send_and_record(env, u_seq, desc=""):
    ...
    print(
        f"  -> {desc}  ({len(u_seq)} steps × {env.dt * 1000:.1f} ms = "
    ...
    for k, u in enumerate(u_seq):
        ...
        if term:
            ...
            break
    return (np.array(t_log), np.array(th_log), np.array(om_log), np.array(u_log))
...
def wait_for_rest(env, timeout=45, tol=0.15):
    ...
    while time.time() - t0 < timeout:
        ...
        time.sleep(env.dt)
    print(
        f" timed out (|ω| never stayed below {tol} for 10 samples — continuing anyway)."
    )
    return False
...
def exp_noise_floor(env, n_samples=600):
    ...
    print(
        f"     omega: mean={stats['omega_mean']:+.4f}  "
        f"std={stats['omega_std']:.4f}  "
        f"peak={stats['omega_peak']:.4f}  rad/s"
    )
    print(
        f"     theta std = {stats['th_std_deg']:.4f}°  "
        f"(expect ~{enc_res / 2:.4f}° if pure quantisation)"
    )
    print(
        f"     encoder step ≈ {enc_res:.4f}°  "
        f"(theoretical {theoretical_enc_deg:.4f}°/count)"
    )
    print(
        f"     encoder tick rate at rest: {tick_rate_hz:.2f} Hz  "
        f"(should be ~0 if truly still)"
    )
    print(
        f"     omega via finite-diff std: {stats['omega_fd_std']:.4f} rad/s  "
...
def exp_step_responses(
    env, amplitudes=(1.0, 2.0, 3.0, -1.0, -2.0, -3.0), step_len=80, settle_steps=60
):
    """
    ...
    for amp in amplitudes:
        ...
        t, th, om, u = send_and_record(env, seq, f"step u={amp:+.1f} V")
...
def estimate_lag_from_steps(step_results, dt):
    ...
    if lags:
        ...
        print(
            f"  Estimated actuation lag: {median_lag:.1f} steps  "
...
def estimate_omega0_from_decay(t, th):
    ...
    print(
        f"  omega0 estimate (small-angle): {omega0_est:.3f} rad/s  "
...
def exp_dead_zone(env, v_range=None, steps_per_v=30):
    ...
    for v in v_range:
        ...
        print(
            f"  v={v:.2f} V  ->  {'MOVED' if did_move else 'still'}  "
    ...
    v_threshold = (
        float(voltages[threshold_idxs[0]]) if len(threshold_idxs) else float("nan")
    )
...
def fit_parameters(step_results, dt, p0=(11.34, 1.33, 28.14, 6.06)):
    """
    ...
    def residuals(params):
        ...
        for t, th, om, u in step_results:
            ...
            n = min(60, len(t))
            th_sim, om_sim = simulate_model(params, t[:n], th[0], om[0], u[:n], dt)
...
def plot_results(
    ...
    fig.suptitle(
        "Unbalanced Disk — System Identification Results",
        fontsize=14,
        fontweight="bold",
    )
    ...
    ax.plot(
        x,
        sp_norm.pdf(x, noise_stats["omega_mean"], noise_stats["omega_std"]),
        "r-",
        lw=1.5,
        label="Gaussian fit",
    )
    ...
    ax.hist(
        th_centered, bins=60, color="coral", edgecolor="white", alpha=0.85, density=True
    )
    ax.set_title("θ noise histogram\n(discrete spikes = encoder limited)")
    ax.set_xlabel("θ − mean [deg]")
    ax.set_ylabel("Density")
    ax.grid(alpha=0.3)
    ...
    ax.set_title(
        f"ω: hardware vs finite-diff on θ\n"
    ...
    ax.set_xlabel("Time [s]")
    ax.set_ylabel("θ [deg]", color="teal")
    ...
    if jitter_ms is not None:
        ...
        ax.axvline(
            dt * 1000, color="r", lw=1.5, ls="--", label=f"nominal {dt * 1000:.1f} ms"
        )
        ax.axvline(
            np.mean(jitter_ms),
            color="orange",
        ...
        ax.set_xlabel("Actual step duration [ms]")
        ax.set_ylabel("Count")
        ax.legend(fontsize=7)
    else:
        ax.text(
    ...
    dz_str = (
        f"{dz_threshold:.2f} V"
        if dz_threshold is not None and not np.isnan(dz_threshold)
        else "not measured"
    )
    jit_str = (
        f"σ={np.std(jitter_ms):.2f} ms, max={np.max(jitter_ms):.1f} ms"
        if jitter_ms is not None
        else "not measured"
    )
...
if __name__ == "__main__":
    ...
    print(
        f"  2. θ obs noise     : σ ≈ {noise_stats['th_std_deg']:.4f}°  (quantisation)"
    )
    print(
        f"     -> Add N(0, {np.deg2rad(noise_stats['th_std_deg']):.5f}) to θ in sim env"
    )
    ...
    print(
        f"  5. dt jitter       : σ={np.std(jitter_ms):.2f} ms, max={np.max(jitter_ms):.1f} ms"
    )
    if np.std(jitter_ms) > 2.0:
        print(
            f"     -> jitter is significant! add Uniform(±{np.std(jitter_ms):.1f}ms) dt noise in sim"
        )
    else:
    ...
    if fit_params:
        ...
        print(
            f"       omega0={fit_params['omega0']:.4f}, gamma={fit_params['gamma']:.4f}, "
